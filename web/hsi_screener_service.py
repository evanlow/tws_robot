"""HSI (Hang Seng Index) Screener Service.

Scans HSI constituents server-side and returns a compact screener payload
...
Usage::
    ...
    result = hsi_screener_service.get_screener_data(refresh=False)
"""
...
_CONSTITUENTS_PATH = Path(__file__).resolve().parent.parent / "data" / "hsi_constituents.csv"
...
class HSIScreenerService:
    """Thread-safe HSI Bollinger Bands screener with in-memory cache.

    Handles HKEX-listed stocks with yfinance ``.HK`` suffix symbols.  The
    ``display_symbol`` field in each row contains the short HKEX code (e.g.
    ``0700``) suitable for display, while ``symbol`` contains the full
    yfinance-compatible ticker (e.g. ``0700.HK``) used for fetching and for
    the analysis drill-down URL.

    Hong Kong stock codes are zero-padded four-digit integers; leading zeros
    are preserved throughout to ensure correct yfinance lookups.
    """
    ...
    def get_screener_data(self, refresh: bool = False) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if not refresh and self._cache is not None and cache_age < _CACHE_TTL_SECONDS:
                logger.debug("Returning cached HSI screener data (age=%.0fs)", cache_age)
                return self._cache
    ...
    def _load_constituents(self) -> List[Dict[str, str]]:
        """Load HSI constituents from the CSV file."""
        if not _CONSTITUENTS_PATH.exists():
            logger.error("HSI constituents file not found: %s", _CONSTITUENTS_PATH)
            return []
    ...
    def _scan_ticker(self, constituent: Dict[str, str]) -> Dict[str, Any]:
        """Fetch price and fundamental data for a single HKEX ticker.
        ...
        base_row = _hsi_insufficient_data_row(constituent)
    ...
    def _scan(self) -> Dict[str, Any]:
        ...
        if not constituents:
            return {
                ...
                "rows": [],
                "error": "HSI constituent list could not be loaded.",
            }

        logger.info(
            "Starting HSI screener scan for %d tickers (workers=%d)…",
            len(constituents),
            _MAX_SCAN_WORKERS,
        )
        t0 = time.monotonic()
        ...
        with ThreadPoolExecutor(max_workers=_MAX_SCAN_WORKERS) as executor:
            ...
            for future in as_completed(future_to_index):
                ...
                try:
                    ...
                    rows[idx] = _hsi_insufficient_data_row(constituents[idx])

        elapsed = time.monotonic() - t0
        logger.info("HSI screener scan completed in %.1fs", elapsed)
        ...
        as_of = datetime.datetime.now(datetime.timezone.utc).isoformat()
        logger.info("HSI screener scan complete: %s", summary)
...
def _hsi_insufficient_data_row(constituent: Dict[str, str]) -> Dict[str, Any]:
    """Build a minimal row for an HSI ticker that could not be scanned."""
    return {
        "symbol": constituent["symbol"],
        "display_symbol": constituent.get("display_symbol", constituent["symbol"].replace(".HK", "")),
...
hsi_screener_service = HSIScreenerService()
