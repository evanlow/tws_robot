...
GET /api/stocks/hsi/screener
    Returns a compact screener payload listing all HSI constituents
...
bp = Blueprint("api_hsi_screener", __name__, url_prefix="/api/stocks")
...
@bp.route("/hsi/screener", methods=["GET"])
def screener():
    """Return the HSI screener payload.
    ...
    try:
        data = hsi_screener_service.get_screener_data(refresh=force_refresh)
    except Exception as exc:
        logger.error("HSI screener scan failed: %s", exc, exc_info=True)
