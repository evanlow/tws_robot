...
class Prompts:
    ...
    STRATEGY_INSIGHT = (
        ...
        "  - Summarise current P&L (dollar amount and percentage). If an "
        "    explicit entry or open date is present in the data, classify the "
        "    holding as short-term momentum, medium-term swing, or long-term "
        "    buy-and-hold using that date. If no entry/open date is present, "
        "    do NOT invent one or make date-based claims; instead infer the "
        "    likely posture only from available fields such as P&L magnitude, "
        "    position size, and any stated targets.\n"
        ...
        "  - TIME TO EXPIRY: For option strategies, DTE (days to expiry) can "
        "    only be computed if the option expiry can be parsed from the "
        "    symbol (e.g., AAPL250620C00200000 → expiry 2025-06-20). If the "
        "    expiry IS parseable, state it explicitly and compute DTE relative "
        "    to today. If the expiry cannot be determined from the data, "
        "    omit all DTE and time-to-expiry commentary and do NOT invent or "
