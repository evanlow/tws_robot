...
- For **paper or live trading**: Interactive Brokers TWS or IB Gateway running with API access enabled (see [TWS Connection Guide](TWS_CONNECTION_GUIDE.md))
...
For developers who want to build their own strategy, see [`docs/runbooks/adding-new-strategy.md`](runbooks/adding-new-strategy.md).
...
See [docs/WEB_API_REFERENCE.md](WEB_API_REFERENCE.md#get-apiaccountcash-availability) for the complete response schema.
...
| State | Meaning |
|-------|---------|
| `OPEN` | Entry order submitted and tracked by the autonomous lifecycle. Depending on broker fill state, this may still be waiting to fill (for example, market closed/pending order) or already filled. |
...
> Note: In this MVP, `OPEN` is lifecycle status, not a strict broker-fill status. Confirm actual fill state in TWS/IBKR order status.
...
`OPEN` in this panel means "active autonomous lifecycle record". If the market is closed, an entry order can remain pending in IBKR while still showing `OPEN` here.
...
**Q: Market is closed and my entry order is pending in IBKR. Why does Autonomous Trades show `OPEN`?**

**A:** This is expected in the current design. `OPEN` indicates the trade is active in the autonomous lifecycle and being tracked. It does not guarantee that IBKR has filled the entry yet. Always use TWS/IBKR order status as the source of truth for fill state.

**Q: If the position fills, will the module close it automatically?**

**A:** The exit logic exists, but it runs only when triggered (for example, the dashboard's **Evaluate Exits Now** button or your own scheduler). There is no always-on background exit loop by default. Exit rules include `RISK_EXIT`, `TIME_EXIT`, `TAKE_PROFIT`, and `STOP_LOSS`.
...
More help:
- [Debugging Strategies Guide](runbooks/debugging-strategies.md)
- [Emergency Procedures](runbooks/emergency-procedures.md)
- [TWS Connection Guide](TWS_CONNECTION_GUIDE.md)
...
See the [TWS Connection Guide](TWS_CONNECTION_GUIDE.md) for step-by-step setup.
...
See [`docs/runbooks/adding-new-strategy.md`](runbooks/adding-new-strategy.md) for a full walkthrough. In brief:
...
| [docs/GETTING_STARTED_30MIN.md](GETTING_STARTED_30MIN.md) | Complete beginner tutorial (30 minutes) |
| [docs/TWS_CONNECTION_GUIDE.md](TWS_CONNECTION_GUIDE.md) | Step-by-step TWS setup guide |
| [docs/QUICK_REFERENCE.md](QUICK_REFERENCE.md) | Commands and config cheat sheet |
| [docs/API_REFERENCE.md](API_REFERENCE.md) | Developer API documentation |
| [docs/WEB_API_REFERENCE.md](WEB_API_REFERENCE.md) | REST API reference for the web dashboard |
| [docs/CONTRIBUTING.md](CONTRIBUTING.md) | How to contribute to TWS Robot |
| [docs/runbooks/debugging-strategies.md](runbooks/debugging-strategies.md) | Troubleshooting guide |
| [docs/runbooks/emergency-procedures.md](runbooks/emergency-procedures.md) | Crisis management procedures |
