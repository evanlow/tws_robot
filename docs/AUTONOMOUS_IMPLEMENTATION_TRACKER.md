...
| Phase 5: Assisted-live protected order-path rehearsal | Complete (Issue #227) | `autonomous/orb_live_order_rehearsal.py` (`build_assisted_live_rehearsal_package`, `ORBAssistedLiveRehearsalPackage`, `ORBAssistedLiveBracketMetadata`, `ORBAssistedLiveRehearsalStore`, `ORBAssistedLiveRefusal`/`ORBAssistedLiveRefusalReason`): turns a valid, still-`PENDING` recommend-only `ORBProposal` into the exact broker-visible protected order package (marketable-`LIMIT` entry + `STOP`/`LIMIT` bracket/OCA metadata, quantity, time-in-force, account id, strategy/session/symbol, entry model, direction, evidence id, readiness/operator-confirmation snapshots, and a unique `audit_event_id`) that assisted-live would submit — **rehearsal/dry-run only**: it never calls a live order-submission API, never calls IBKR `placeOrder`, never flips the live master switch, and never starts a continuous live loop. Fails closed (raising `ORBAssistedLiveRefusal` with a structured reason) on a missing proposal; a stale/expired/skipped/already-executed proposal or any other proposal-safety-invariant failure; a missing stop or target; the Phase 4 (#213) live-readiness result not being exactly `ASSISTED_LIVE_CANDIDATE`; the live master switch being disabled; missing explicit operator confirmation; an account id mismatch (or either id missing); a non-`LONG` direction; `MODEL_C_REVERSAL`/unknown entry models; a non-`LIMIT` (raw market) order type; incomplete symbol/session/strategy evidence; and an unrepresentable bracket (zero/invalid quantity or an invalid stop/entry/target ordering). Every build attempt — success or refusal — is audit-logged (`kind="orb_assisted_live_rehearsal"`, actions `rehearsal_created`/`rehearsal_refused`). API (`web/routes/api_opening_range.py`): `POST /api/orb/strategies/<name>/assisted-live/rehearse` (re-evaluates Phase 4 assisted-live readiness from the same evidence/connection/confirmation sources as `GET .../live-readiness` via a new shared `_compute_orb_live_readiness()` helper, then builds the package or returns `409` with the structured refusal reason), `GET /api/orb/strategies/<name>/assisted-live/rehearsals` (list), `GET /api/orb/assisted-live/rehearsals/<rehearsal_id>` (fetch one); no live-submit endpoint exists in this phase. Tests `tests/test_orb_live_order_rehearsal.py` (20, incl. success path, Model A/B eligibility, and every fail-closed reason) + `tests/test_orb_assisted_live_api.py` (10, incl. 404/400 request-shape checks, default-locked refusal, account-mismatch refusal, a market-data-source query-override regression, and a monkeypatched full-pass success/list/get/audit-log round trip). Review follow-up: `_compute_orb_live_readiness()` now always evaluates `AutonomousLiveRunnerConfig.live_market_data_provider` for the `market_data_source_acceptable` gate; a `market_data_source` query-string value can never replace or rescue an unsafe actual live-runner source and is only surfaced as a separate `simulated_market_data_source` diagnostic (regression tests added in `tests/test_orb_live_readiness_api.py` and `tests/test_orb_assisted_live_api.py`). No fully autonomous live trading, no continuous live loop, no short entries, no Model C execution, no futures/forex, no automatic paper-to-live promotion. |
| Phase 6: Human-confirmed tiny-live assisted execution | Complete (Issue #229) | `autonomous/orb_live_order_adapter.py` (`ORBLiveOrderAdapter`, `ORBLiveBracketOrderRequest`, `ORBLiveBrokerSubmission`, `RefusingLiveOrderAdapter`, `FakeORBLiveOrderAdapter`): narrow, mockable interface for exactly one order shape — a protected long bracket/OCA group (`submit_protected_long_bracket`/`is_protection_broker_visible`/`cancel_if_pending`); no generic "place any order" method exists. The production default (`RefusingLiveOrderAdapter`) fails closed — it refuses every submit and never reports protection as broker-visible — until a real broker adapter is explicitly wired in via `app.config["orb_live_order_adapter"]`; tests use `FakeORBLiveOrderAdapter` and never call a real broker. `autonomous/orb_tiny_live_execution.py` (`submit_tiny_live_order`, `ORBTinyLiveOrderGroup`, `ORBTinyLiveOrderStore`, `ORBTinyLiveRefusal`/`ORBTinyLiveRefusalReason`): submits a validated, not-previously-submitted Phase 5 (#227) `DRY_RUN_ONLY` rehearsal package as a real tiny-live order group only after an explicit final human confirmation (`confirm_live_order=true` + named `operator`). Re-checks, immediately before calling the broker adapter: the rehearsal is still `DRY_RUN_ONLY` and not already submitted; the proposal still exists, is still `PENDING`, and still matches the rehearsal's proposal id/strategy/session/symbol; Phase 4 (#213) readiness is still exactly `ASSISTED_LIVE_CANDIDATE`; the live master switch is enabled; the emergency stop is not active; the connected account id matches the operator-confirmed `expected_account_id`; the market-data source is acceptable; the daily live ORB trade cap (default 1) is not exceeded; verified account equity is available and the tiny-live cash cap (`<=1%` of equity) is not exceeded (an unverifiable equity fails closed as `risk_cap_unverifiable`, never defaults to "safe"); the entry is `LIMIT` (never a raw market order) with a valid `stop < entry < target` bracket; direction is `LONG`; and the entry model is Model A/B only (Model C/unknown models refused). Only once every gate passes does it call `adapter.submit_protected_long_bracket()`; if the adapter cannot confirm broker-visible protection, the order group is cancelled immediately via `adapter.cancel_if_pending()` and the submit still fails closed (`protection_not_broker_visible`). Every submit attempt — success or refusal — is audit-logged (`kind="orb_tiny_live_execution"`, actions `tiny_live_order_submitted`/`tiny_live_order_refused`) with strategy/session/symbol/account id/rehearsal id/proposal id/order group id/broker order ids/entry-stop-target prices/quantity/readiness snapshot/operator-confirmation payload/refusal reason. A successful submit also transitions the source proposal to `EXECUTED` via `ORBProposalStore.mark_executed()` so it can never be independently paper-executed or resubmitted. API (`web/routes/api_opening_range.py`): `POST /api/orb/assisted-live/rehearsals/<rehearsal_id>/submit-tiny-live` (requires `confirm_live_order`/`expected_account_id`/`operator`/`notes`; re-evaluates readiness via the same shared `_compute_orb_live_readiness()` helper as Phases 4/5, reads live emergency-stop/market-data/account-equity state fresh, and returns `201` with the order group or `409` with the structured refusal reason), `GET /api/orb/tiny-live/orders` (list), `GET /api/orb/tiny-live/orders/<order_group_id>` (fetch one), `POST /api/orb/tiny-live/orders/<order_group_id>/cancel-if-pending` (idempotent; only cancels while `status == SUBMITTED`). `ORBTinyLiveOrderStore` durably persists to `<orb_evidence_dir>/orb_tiny_live_orders.json` (atomic write-then-rename): immediately before calling the broker adapter, `submit_tiny_live_order()` writes a durable `PENDING_SUBMIT` reservation for the rehearsal so a process crash between a successful broker call and this process recording that success cannot leave the rehearsal/proposal resubmittable — `is_rehearsal_submitted()` treats `PENDING_SUBMIT`, `SUBMITTED`, and `SUBMITTED_NEEDS_PROPOSAL_RECONCILIATION` as durably blocking, even after the in-memory store is rebuilt from disk. A genuine broker failure (adapter raised, or protection not broker-visible) releases the reservation so a legitimate retry remains possible. On success, `finalize_submitted()` durably persists the broker order ids/status before the function returns. If the API's subsequent `ORBProposalStore.mark_executed()` call fails, the order group is flagged `SUBMITTED_NEEDS_PROPOSAL_RECONCILIATION` via `mark_needs_reconciliation()` (durable, still blocking) instead of only being logged. Tests `tests/test_orb_tiny_live_execution.py` (35, incl. success path, every fail-closed reason, protection-not-broker-visible cancel-and-refuse, broker-submit-failure handling — all proving no broker call happens on refused paths — plus durable-idempotency regression tests simulating in-memory-store loss after a successful submit, a reservation with no finalized group, broker-failure reservation release, and reconciliation flagging) + `tests/test_orb_tiny_live_api.py` (19, incl. 404s, successful submit with broker ids, double-submit/daily-cap/account-mismatch/operator-confirmation/master-switch/readiness/emergency-stop/market-data/equity-unverifiable/protection-not-visible refusals, the safe-by-default `RefusingLiveOrderAdapter` when no fake adapter is wired in, cancel-if-pending, a simulated process-restart resubmit refusal, and a `mark_executed()`-failure reconciliation test). Not fully autonomous: no continuous live loop, no automatic promotion from readiness/rehearsal to submit — every call requires an explicit HTTP request with a fresh operator confirmation. No short entries, no Model C execution, no forex/futures, no raw market orders, no size escalation beyond tiny-live caps. |
